...
def parse(line):
    ...
    try:
        ...
        temp = [destination_code, destination_name, subcode, vessel_id, vessel_name, cruise_id, cruise_line_name,
                ...
                oceanview_bucket_price, balcony_bucket_price, suite_bucket_price, ports_string[1:]]
    except IndexError:
        temp = [destination_code, destination_name, subcode, vessel_id, vessel_name, cruise_id, cruise_line_name,
                itinerary_id, brochure_name, number_of_nights, sail_date, return_date, "N/A",
                "N/A", "N/A", "N/A", ports_string[1:]]
...
def write_file_to_excell(data_array):
    ...
    path_to_file = userhome + '\\Dropbox\\XLSX\\' + str(now.year) + '-' + str(now.month) + '-' + str(
        ...
        now.day) + ' - Cruise only price Oceania Cruises.xlsx'
    ...
    worksheet.write('Q1', 'PortList', bold)
    row_count = 1
...
def write_ports_to_excell(data_array):
    ...
    now = datetime.datetime.now()
    path_to_file = userhome + '\\Dropbox\\XLSX\\' + str(now.year) + '-' + str(now.month) + '-' + str(
        now.day) + '/' + str(now.year) + '-' + str(now.month) + '-' + str(now.day) + '- OceaniaCOP.xlsx'
    if not os.path.exists(userhome + '\\Dropbox\\XLSX\\' + str(now.year) + '-' + str(
            now.month) + '-' + str(now.day)):
        os.makedirs(
            userhome + '\\Dropbox\\XLSX\\' + str(now.year) + '-' + str(now.month) + '-' + str(now.day))
