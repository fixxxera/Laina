...
def parse(de):
    try:
        ...
        count = int(session.get(url=url, proxies=proxies, headers=headers).json()['Count'])
        pages = math.ceil(count / 5)

    except JSONDecodeError:
        try:
            url = 'http://www.cunard.com/cruise-search/book-a-cruise/results/?tids=' + de + '&pg=1&rt=&sort=&ajax=1'
            count = int(session.get(url=url, proxies=proxies).json()['Count'])
            pages = math.ceil(count / 5)
        except JSONDecodeError:
    ...
    while index <= int(pages):
        try:
            url = 'http://www.cunard.com/cruise-search/book-a-cruise/results/?tids=' + de + '&pg=' + str(
                index) + '&rt=&sort='
            page = session.get(url=url, proxies=proxies).json()['Html']
        except JSONDecodeError:
            try:
                url = 'http://www.cunard.com/cruise-search/book-a-cruise/results/?tids=' + de + '&pg=' + str(
        ...
        for result in results_on_page:
            ...
            if 'http' in button_url:
                ...
                prices_page = session.get(url=button_url, proxies=proxies).text
