...
# proxy = get_proxy()
response = requests.get("https://www.ncl.com/search_vacations?cruise=1&cruiseTour=0&cruiseHotel=0&cruiseHotelAir=0&flyCruise=0&numberOfGuests=4294953449&state=undefined&pageSize=10&currentPage=")
...
def send_req(link):
    print(link)
    response = session.post(link, headers=headers).json()
...
def split_carib_auto(ports, dc, dn):
    ...
    ec = []
    bm = []
    conn = sqlite3.connect('PortsExplorer\\ports.db')
    c = conn.cursor()
...
def parse(c):
    ...
    try:
        if "Hotel Bundle" in brochure_name:
            return
    except TypeError:
        return
    ...
    page = session.post(price_url, headers=headers)
    ...
    for each in cruise_results['results']:
        ...
        ports = {}
        try:
            ports = each['Record']['Properties']['p_Sailing_Port_Sequence']
        except KeyError:
            print('Still Error')
        portlist = []
        final_ports = ""
        ...
        temp = [destination_code, destination_name, vessel_id, vessel_name, cruise_id, cruise_line_name, itinerary_id,
                ...
                interior_bucket_price, oceanview_bucket_price, balcony_bucket_price, suite_bucket_price, final_ports]
        tmp2 = [temp]
        print(temp)
