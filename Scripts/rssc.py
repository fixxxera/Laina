...
proxies = get_proxy()
page = requests.get('https://www.rssc.com/cruises', proxies=proxies)
...
def get_from_code(dc):
    ...
    if dc == 'GRNDV':
        return ['Exotics', 'O']
    if dc == "SOPAC":
        return ['South Pacific', 'TBD']
    if dc == 'TRANS':
        return ['TransAtlantic', 'TBD']
    if dc == "CUBA":
        return ["Cuba", "C"]
    if dc == "BERMU":
        return ["Bermuda", "BM"]
...
def split_carib_auto(ports, dc, dn):
    ...
    ec = []
    bm = []
    filename = os.path.join(fileDir, 'PortsExplorer\\ports.db')
...
def parse(v):
    ...
    h1 = detail_soup.find_all('div', {'class': 's3p-info'})[1]
    h1date = h1.contents[3].text
    h1year = h1.contents[5].text
    date = h1date.split(' ')[0] + "/" + h1date.split(" ")[1] + "/" + h1year
    date = date.split("/")
            ...
            for r in l:
                try:
                    if column_count == 0:
                        ...
                        worksheet.write_string(row_count, column_count, str(r), ordinary_number)
                        column_count += 1
                    elif column_count == 3:
                        worksheet.write_string(row_count, column_count, str(r), centered)
                        column_count += 1
